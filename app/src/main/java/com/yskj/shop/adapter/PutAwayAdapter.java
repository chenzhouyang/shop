...
public class PutAwayAdapter extends RecyclerView.Adapter<PutAwayAdapter.MyViewHolder> {
    private Context context;
    private List<PutAwayEntity.DataBean> list;
    private OnItemClickListener mOnItemClickListener;
    public PutAwayAdapter(Context context, List<PutAwayEntity.DataBean> list){
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        PutAwayEntity.DataBean putdate = list.get(position);
        holder.goods_price.setText("¥"+ StringUtils.getStringtodouble(putdate.price));
        GlideImage.loadImage(context,holder.img_goods,putdate.original.replaceAll("fs:", Ips.API_SHOPURL+"/statics"), R.mipmap.img_error);
