...
import com.yskj.shop.PgyApplication;

import java.io.File;
import java.math.BigDecimal;

/**
 * 创建日期 2017/4/24on 14:52.
 * 描述：
 * 作者：姜贺YSKJ-JH
 */
...
public class GlideCatchUtil {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(PgyApplication.getInstance().getCacheDir() + "/" + GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(PgyApplication.getInstance().getCacheDir() + "/" + GLIDE_CARCH_DIR, true);
    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(PgyApplication.getInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(PgyApplication.getInstance()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(PgyApplication.getInstance()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
